...
describe("§2 — the walk-in seat", () => {
  it("a door on the GROUP row beside `Sell a course`, by the existing `calendar.book` key ⇒ the calendar opens the SAME create form locked to the row", () => {
    ...
    expect(ACTION_KEYS_SNAPSHOT.length).toBe(58) /* TASK-427 + TASK-429: budget-view, other-cancel-all */; // no new key from THIS task (TASK-402's four, TASK-407's and TASK-412's one came after)
  });
