...
describe("§2 — the course card", () => {
  ...
  it("the 47th key and the copy: `action:bookings.course-rental` in the snapshot (TASK-390's name); rental keys +9 ×2", () => {
    ...
    expect(ACTION_KEYS_SNAPSHOT.length).toBe(57) /* TASK-427: + teachers.budget-view */; // 47 + student-archive + other-series + group-series + TASK-402's four camp keys + TASK-407's teacher-leave
