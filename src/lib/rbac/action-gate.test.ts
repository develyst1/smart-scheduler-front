...
describe("§1 — ONE gate, everywhere", () => {
  ...
  it("the snapshot: 54 keys in 9 areas (50 + TASK-401's four `camp.*`); every key a site uses exists; exactly two keys have no FE site", () => {
    expect(KEYS.length).toBe(58) /* TASK-427 + TASK-429: budget-view, other-cancel-all */; // + TASK-406/407's `calendar.teacher-leave` + TASK-411/412's `people.parent-archive`
  ...
  it("the sweep: 86 key literals across 33 files (78 + TASK-402's camp doors: open/edit/close a week, sell ×2, redeem ×2, mark)", () => {
    expect(sites.length).toBe(95); /* TASK-427: the budget doors ask both keys · TASK-429: the Manage-plan page asks its four grants */ // + TASK-407's `Report leave` door (CalendarContent) + TASK-412's parent archive/restore doors (People) + TASK-419's camp `Swap teacher` (CampBlockPanel)
    expect(new Set(sites.map((s) => s.file)).size).toBe(36); /* + OtherSeriesContent */
