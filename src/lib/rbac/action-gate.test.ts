...
describe("§1 — ONE gate, everywhere", () => {
  ...
  it("the snapshot: 54 keys in 9 areas (50 + TASK-401's four `camp.*`); every key a site uses exists; exactly two keys have no FE site", () => {
    expect(KEYS.length).toBe(59) /* TASK-427 + TASK-429 + TASK-432: budget-view, other-cancel-all, coach-rate */; // + TASK-406/407's `calendar.teacher-leave` + TASK-411/412's `people.parent-archive`
