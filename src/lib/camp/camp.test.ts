...
describe("§2 — the wire and the doors", () => {
  ...
  it("the keys: the four doors by their acts; `menu:camp` in the registry after Badges and on the nav; hidden not disabled", () => {
    ...
    expect(ACTION_KEYS_SNAPSHOT.length).toBe(59) /* TASK-427 + TASK-429 + TASK-432: budget-view, other-cancel-all, coach-rate */; // + TASK-407's teacher-leave + TASK-412's parent-archive
  ...
  it("copy: camp 60 · nav.camp — both languages; the Camp card door on every student row", () => {
    ...
    expect(Object.keys(en).length).toBe(76); // TASK-402: 60 · TASK-404: +9 (undo ×7, qr ×2) · TASK-419: +7 (the window + per-day editor)
