...
import BookingModal from "@/components/partials/Calendar/Modal/BookingModal";
...
/**
 * REQ-101 / SPEC-088 Part A (TASK-429) — the Manage-plan page for an ECA/Free/KOL series (`/scheduler/other/:key`,
 * `menu:calendar`). The header (title · kind · heads · time · the teachers with their rates), the rows in date order
 * with their status chips, and the series doors — each hidden without its key (`seriesDoors`, pure): Confirm all
 * (`status`), Cancel all (key 58), Add / Remove / Swap teacher (`booking-edit`, `fromDate` default today), Add dates
 * (`other-series`), Edit header (`booking-edit`). A row opens the EXISTING booking modal (by bookingId) for its own
 * doors. Every 409 is the server's sentence; the page refetches only on 2xx.
 */
export default function OtherSeriesContent({ seriesKey }: { seriesKey: string }) {
  const t = useT();
  const can = useCan();
  ...
  const rate = (id: string) => (typeof series?.teacherRates?.[id] === "number" ? ` · ฿${(series.teacherRates[id] / 100).toLocaleString("th-TH")}` : "");
  ...
  return (
    <Stack gap="md" data-series={series.key}>
      ...
      {selected && <BookingModal isOpen onClose={() => setSelected(undefined)} booking={selected} teachers={teachers} bookings={page?.items ?? []} onOverbook={() => {}} onWalkIn={() => {}} />}
